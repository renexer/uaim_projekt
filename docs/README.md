...
Baza domyślna: SQLite przez `sqlite:///app.db`. Produkcyjnie można użyć `DATABASE_URL`, np. PostgreSQL, bo w zależnościach jest `psycopg2-binary`.
...
│   ├── routes.py                      # starszy /api/health, nie jest rejestrowany w create_app
...
| `MAIL_BACKEND` | `console` | `console` wypisuje maile do stdout; inne backendy nie są zaimplementowane |
